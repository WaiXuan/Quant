...
class PerpBitget():
    def __init__(self, test, apiKey=None, secret=None, password=None):
        bitget_auth_object = {
            'apiKey'    :   apiKey,
            'secret'    :   secret,
            'password'  :   password
...
# region 持倉
    ...
    # region 查詢持倉(實時)
    #@authentication_required
    def fetch_position(self, productType, symbol, marginCoin):
        try :                            
            request = {
            ...
            # result = self._session.fetch_position(request)
            result = self._session.fetch_position(productType=productType, symbol=symbol, marginCoin=marginCoin)

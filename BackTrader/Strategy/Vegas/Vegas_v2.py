...
class MyStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        long_signal = False
        short_signal = False
        long_ema_signal = False
        short_ema_signal = False

        if self.bt_ema12[-1] > self.bt_ema144[-1] and self.bt_ema144[-1] > self.bt_ema676[-1] :
            long_signal = True
        if self.bt_ema12[-1] < self.bt_ema144[-1] and self.bt_ema144[-1] < self.bt_ema676[-1] :
            short_signal = True      
        if self.bt_ema144[-1] > self.bt_ema169[-1] and self.bt_ema169[-1] > self.bt_ema576[-1] and self.bt_ema576[-1] > self.bt_ema676[-1] :
            long_ema_signal = True
        if self.bt_ema144[-1] < self.bt_ema169[-1] and self.bt_ema169[-1] < self.bt_ema576[-1] and self.bt_ema576[-1] < self.bt_ema676[-1] :
            short_ema_signal = True       
        # endregion
        
        # region 開倉條件--------------------------------------------------------
        if not self.position :
            if((long_ema_signal and self.bt_ema12[-2] < self.bt_ema144[-1] and self.bt_ema12[-1] > self.bt_ema144[-1]) or
               (long_signal and self.data.close[-1] > self.bt_ema144[-1] and self.data.low[-1] < self.bt_ema144[-1])):
                size = self.broker.get_cash()/self.data.open[0] *0.98
            ...
            if ((short_ema_signal and self.bt_ema12[-2] < self.bt_ema144[-1] and self.bt_ema12[-1] > self.bt_ema144[-1]) or
                (short_signal and self.data.close[-1] < self.bt_ema144[-1] and self.data.high[-1] > self.bt_ema144[-1])):
                size = self.broker.get_cash()/self.data.open[0] *0.98
        ...
        if self.position :             
            if self.getposition().size > 0 :  
                ...
                #     # print( "getposition ",self.getposition().size * self.pricein)  
                #     # print( available_cash/ (self.getposition().size * self.pricein * (-1) ))
                #     if available_cash / (self.getposition().size * self.pricein) > 1 :
                #         size = self.broker.get_cash()/self.data.open[0] *0.98
                #         self.order = self.buy(size=size,price=self.data.open[0])                    
                #         print(f"做多補倉時間:{self.datein} 開倉價格:{self.pricein} size:{self.getposition().size}")        
                                                            
                if self.bt_ema12[-1] < self.bt_ema169[-1] : # ema12小於ema676止損
                    self.log_out(size=self.getposition().size, reason="ema12小於ema144止損")
                    self.order = self.sell(size=self.getposition().size, price=self.data.close[0])   
                elif self.data.close[-1] < self.bt_ema676[-1]:
                    self.log_out(size=self.getposition().size, reason="收盤價小於ema676止損")
                    self.order = self.sell(size=self.getposition().size, price=self.data.close[0])                                                                                 
            ...
            if self.getposition().size < 0 :
                # if ((short_ema_signal and self.bt_ema12[-2] < self.bt_ema144[-1] and self.bt_ema12[-1] > self.bt_ema144[-1]) or
                #     (short_signal and self.data.close[-1] < self.bt_ema144[-1] and self.data.high[-1] > self.bt_ema144[-1])):
                #     available_cash = self.broker.get_cash() / 2 - self.getposition().size * self.pricein * (-1)
                    
                #     # print("available_cash",available_cash)
                #     # print( "getposition ",self.getposition().size * self.pricein * (-1) )       
                #     # print( available_cash / (self.getposition().size * self.pricein * (-1) ))
                                   
                #     if available_cash / (self.getposition().size * self.pricein * (-1)) > 1 :
                #         size = self.broker.get_cash()/self.data.open[0] *0.98
                #         self.sell(size=size,price=self.data.open[0])
                                                                        
                if self.bt_ema12[-1] > self.bt_ema169[-1]:  # ema12大於ema676止損
                    self.log_out(size=self.getposition().size, reason="ema12大於ema169止損")
                    self.buy(size=self.getposition().size)      
                elif self.data.close[-1] > self.bt_ema676[-1]:  # 收盤價大於ema676止損
                    self.log_out(size=self.getposition().size, reason="收盤價大於ema676止損")
                    self.buy(size=self.getposition().size)            
